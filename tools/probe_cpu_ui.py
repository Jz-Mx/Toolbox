...
def probe(window):
    for i in range(5):
        time.sleep(3)
        print(f"  t={i*3}s hpCpu={ev_js(window, 'document.getElementById(\"hpCpu\").textContent')} cores={ev_js(window, 'document.getElementById(\"dCores\").textContent')}", flush=True)
    print("PROBE_CPU_UI_DONE", flush=True)

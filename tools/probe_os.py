# -*- coding: utf-8 -*-
"""验证：系统信息详细版本（Windows 10 22H2 专业版）。"""
import os
import sys
import threading
import time
...
def probe(window):
    ...
    api = TalentAPI()
    s = api.get_sysinfo()
    print("PY os:", s.get("os"), flush=True)
    print("UI rows:", ev_js(window, "[...document.querySelectorAll('.detail-panel .d-item')].map(d => d.querySelector('span').textContent + '=' + d.querySelector('b').textContent).join(' | ')"), flush=True)
    print("PROBE_OS_DONE", flush=True)
